...
class Agent:
    ...
    def learn(self, batch, gamma):
        ...
        for i in range(self.n_agents):
            # subset states, actions, rewards, next_states and dones of agent i
            state = states[i,:,:].reshape(BATCH_SIZE, self.state_size)
            action = actions[:,i,:].reshape(BATCH_SIZE, self.action_size)
            reward = rewards[:,i,:].reshape(BATCH_SIZE, 1)
            next_state = next_states[i,:,:].reshape(BATCH_SIZE, self.state_size)
            done = dones[:,i,:].reshape(BATCH_SIZE, 1)
            
            # compute critic loss
            Q_local = self.local_critic(state, action)
            ...
            Q_local = self.local_critic(state, self.local_actor(state))
...
class ReplayBuffer:
    ...
    def sample(self):
        ...
        states = torch.from_numpy(np.dstack([e.state for e in experiences if e is not None])).float().to(device)
        actions = torch.from_numpy(np.dstack([e.action for e in experiences if e is not None])).float().to(device)
        rewards = torch.from_numpy(np.dstack([e.reward for e in experiences if e is not None])).float().to(device)
        next_states = torch.from_numpy(np.dstack([e.next_state for e in experiences if e is not None])).float().to(device)
        dones = torch.from_numpy(np.dstack([e.done for e in experiences if e is not None]).astype(np.uint8)).float().to(device)
